...
- 支持图片缓存，解决部分地区`pixiv`打开慢或打不开的问题
...
#### API服务
[缓存缩略图URL+详情页URL](https://cloud.mokeyjay.com/pixiv/pixiv.json)（推荐）  
...
### 方案二：自行架设服务
适用于动手能力较强或需要自定义的用户  
由于Pixiv已经被墙，所以如果想要部署在中国大陆内，请正确配置`$download_proxy`代理  
...
为了更好的显示效果，建议配置一个定时任务，每天0点触发`download.php`  
